...
const app_es = extension.import('service:es:app');
const DEFAULT_LIMIT = 100;
const MAX_LIMIT = 1000;
const MAX_OFFSET = 100_000;
const parseIntegerParam = (value, { key, min, max, fallback }) => {
...
extension.on('create.drivers', event => {
    event.createDriver('app-telemetry', 'app-telemetry', {
        async get_users ({ app_uuid, limit, offset }) {
            ...
            const users = await db.read(`SELECT user.username, user.uuid FROM user_to_app_permissions 
                    INNER JOIN user ON user_to_app_permissions.user_id = user.id  
                    WHERE permission = 'flag:app-is-authenticated' AND app_id=? ORDER BY (dt IS NOT NULL), dt, user_id LIMIT ? OFFSET ?`, [result.private_meta.mysql_id, safeLimit, safeOffset]);
            return users.map(e => {
                return { user: e.username, user_uuid: e.uuid };
            });
        },
        async user_count ({ app_uuid }) {
            // first lets make sure executor owns this app
            const [result] = (await app_es.select({ predicate: new Eq({ key: 'uid', value: app_uuid }) }));
            if ( ! result ) {
                throw APIError.create('permission_denied');
            ...
            const [data] = await db.read(`SELECT count(*) FROM user_to_app_permissions 
                    WHERE permission = 'flag:app-is-authenticated' AND app_id=?;`, [result.private_meta.mysql_id]);
...
export {};
//# sourceMappingURL=app-user-count.js.map