...
import { SystemActorType } from '@heyputer/backend/src/services/auth/Actor.js';
...
export class DynamoKVStore {
    ...
    constructor ({ ddbClient, sqlClient, tableName, meteringService }) {
        this.#ddbClient = ddbClient;
        this.#sqlClient = sqlClient;
        this.#tableName = tableName;
        this.#meteringService = meteringService;
        this.#enableMigrationFromSQL = !this.#ddbClient.config?.aws;
    }
    ...
    async get ({ key }) {
        ...
        for ( const key of keys ) {
            ...
            if ( this.#enableMigrationFromSQL ) {
                ...
                const kv_row = await this.#sqlClient.read('SELECT * FROM kv WHERE user_id=? AND app=? AND kkey_hash=? LIMIT 1', [user.id, app?.uid ?? DynamoKVStore.LEGACY_GLOBAL_APP_KEY, key_hash]);
                if ( kv_row[0]?.value ) {
                    (async () => {
                        await this.set({ key: kv_row[0].key, value: kv_row[0].value });
                        await this.#sqlClient.write('DELETE FROM kv WHERE user_id=? AND app=? AND kkey_hash=?', [user.id, app?.uid ?? DynamoKVStore.LEGACY_GLOBAL_APP_KEY, key_hash]);
                    })();
    ...
    async #getBatches (namespace, allKeys) {
        const batches = [];
    ...
    async list ({ as, limit, cursor, pattern }) {
        const actor = Context.get('actor');
        const app = actor.type?.app ?? undefined;
        const user = actor.type?.user ?? undefined;
        if ( ! user )
        ...
        const entriesRes = await this.#ddbClient.query(this.#tableName, { namespace }, normalizedLimit ?? 0, pageKey, '', false, normalizedPattern ? { beginsWith: { key: 'key', value: normalizedPattern } } : undefined);
        ...
        if ( this.#enableMigrationFromSQL && !paginated ) {
            const oldEntries = await this.#sqlClient.read('SELECT * FROM kv WHERE user_id=? AND app=?', [user.id, app?.uid ?? DynamoKVStore.LEGACY_GLOBAL_APP_KEY]);
        ...
        if ( paginated ) {
            const nextCursor = this.#encodeCursor(entriesRes.LastEvaluatedKey);
            if ( nextCursor ) {
    ...
    async flush () {
        ...
        const namespace = this.#getNameSpace(actor);
        const entriesRes = await this.#ddbClient.query(this.#tableName, { namespace });
        ...
        if ( this.#enableMigrationFromSQL ) {
            await this.#sqlClient.write('DELETE FROM kv WHERE user_id=? AND app=?', [user.id, app?.uid ?? DynamoKVStore.LEGACY_GLOBAL_APP_KEY]);
        }
        return !!allRes;
    }
    ...
    async #createPaths (namespace, key, pathList) {
        ...
        const objectsEqual = (left, right) => {
            ...
            for ( const key of leftKeys ) {
                ...
                if ( ! objectsEqual(left[key], right[key]) )
                {
                    return false;
                }
            }
        ...
        for ( const layerPath of orderedPaths ) {
            ...
            const layerUpsertRes = await this.#ddbClient.update(this.#tableName, { key, namespace }, `SET ${attrName} = if_not_exists(${attrName}, ${valueToken})`, expressionValues, expressionNames);
    ...
    async incr ({ key, pathAndAmountMap }) {
        ...
        const res = await this.#ddbClient.update(this.#tableName, { key, namespace }, `SET ${[...setStatements].join(', ')}`, valueAttributeValues, { ...valueAttributeNames, '#value': 'value' });
    ...
    async decr ({ key, pathAndAmountMap }) {
    ...
    async remove ({ key, paths }) {
        ...
        try {
            const res = await this.#ddbClient.update(this.#tableName, { key, namespace }, `REMOVE ${removeStatements.join(', ')}`, undefined, { ...valueAttributeNames, '#value': 'value' });
            this.#meteringService.incrementUsage(actor, 'kv:write', res?.ConsumedCapacity?.CapacityUnits ?? 1);
            return res.Attributes?.value;
        }
        catch (e) {
            const message = e?.message ?? '';
            if ( e?.name === 'ValidationException' && /document path|invalid updateexpression/i.test(message) ) {
    ...
    async update ({ key, pathAndValueMap, ttl }) {
        if ( !pathAndValueMap || Object.keys(pathAndValueMap).length === 0 ) {
    ...
    async #expireAt (key, timestamp) {
        ...
        const res = await this.#ddbClient.update(this.#tableName, { key, namespace }, 'SET #ttl = :ttl, #value = if_not_exists(#value, :defaultValue)', { ':ttl': timestamp, ':defaultValue': null }, { '#ttl': 'ttl', '#value': 'value' });
        this.#meteringService.incrementUsage(actor, 'kv:write', res?.ConsumedCapacity?.CapacityUnits ?? 1);
    }
}
__decorate([
    Span('kv:get'),
], DynamoKVStore.prototype, 'get', null);
__decorate([
    Span('kv:set'),
], DynamoKVStore.prototype, 'set', null);
__decorate([
    Span('kv:del'),
], DynamoKVStore.prototype, 'del', null);
__decorate([
    Span('kv:list'),
], DynamoKVStore.prototype, 'list', null);
__decorate([
    Span('kv:flush'),
], DynamoKVStore.prototype, 'flush', null);
__decorate([
    Span('kv:expireAt'),
], DynamoKVStore.prototype, 'expireAt', null);
__decorate([
    Span('kv:expire'),
], DynamoKVStore.prototype, 'expire', null);
__decorate([
    Span('kv:incr'),
], DynamoKVStore.prototype, 'incr', null);
__decorate([
    Span('kv:add'),
], DynamoKVStore.prototype, 'add', null);
__decorate([
    Span('kv:remove'),
], DynamoKVStore.prototype, 'remove', null);
__decorate([
    Span('kv:update'),
], DynamoKVStore.prototype, 'update', null);
//# sourceMappingURL=DynamoKVStore.js.map