...
export class OpenAiImageGenerationProvider {
    ...
    async generate ({ prompt, quality, test_mode, model, ratio }) {
        ...
        const hasInputTokenUsage = usage.inputTokens > 0 ||
    ...
    #extractUsage (result) {
        ...
        const cachedInputTokens = Math.max(this.#toSafeCount(usage.cached_input_tokens), this.#toSafeCount(inputDetails.cached_tokens));
        const cachedDetails = ((inputDetails.cached_tokens_details || inputDetails.cache_tokens_details) ?? {});
    ...
    #calculateInputCostInCents (selectedModel, usage) {
    ...
    #calculateOutputCostInCents (selectedModel, usage, fallbackPriceInCents) {
        if ( ! this.#isGptImageModel(selectedModel.id) ) {
    ...
    #setResponseCostMetadata ({ model, quality, ratio, inputCostInCents, outputCostInCents, usage, inputUsageSource, outputUsageSource, outputPriceInCents }) {
    ...
    #costForTokens (tokenCount, centsPerMillion) {
    ...
    #buildPriceKey (model, quality, size) {
    ...
    #buildApiParams (model, baseParams) {
        const apiParams = {
...
//# sourceMappingURL=OpenAiImageGenerationProvider.js.map